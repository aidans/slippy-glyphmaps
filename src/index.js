...
function glyphMap(options) {
    ...
    const colourFieldFn = options.colourFieldFn?options.colourFieldFn:(row) => row.value;
    ...
    const useTransparency=options.useTransparency?options.useTransparency:true;
    const colourScale = options.colourScale
      ? options.colourScale
      : d3.scaleSequential(d3.interpolateYlGn);
    const interactiveZoomPan=options.interactiveZoomPan!==undefined?options.interactiveZoomPan:true;  
    ...
    const drawSamplePoints=options.drawSamplePoints?options.drawSamplePoints:false;
    let reduceMaup=options.reduceMaup!==undefined?options.reduceMaup:false;
  ...
  if (options.customMap){ //deprecated
  ...
  if (options.glyph){
    ...
    aggrFn = (cell,row,weight) => (cell.value = (cell.value ?? 0) + weight); //count
  else if (type == "sum"){ //assume weight will be 0-1
      aggrFn = (cell,row,weight) => (cell.value = (cell.value ?? 0) + colourFieldFn(row)*weight); //sum
  }
  else if (type == "mean"){
      aggrFn = (cell,row,weight) => { //assume weight will be 0-1
...
//tooltip
  if (tooltipTextFn) {
    ...
    panel.addEventListener("mousemove", (e) => {
      ...
      if (grid && discretiser){
        const [col,row]=discretiser.getColRow(e.offsetX-grid.xOffset,e.offsetY-grid.yOffset);
          if (
            col > 0 &&
            row > 0 &&
    ...
    panel.addEventListener("keydown", (event) => {
      ...
      if (
        (interactiveCellSize && event.keyCode == 38 && !event.shiftKey)//UP
      ) {
      ...
      if (
        (interactiveCellSize && event.keyCode == 40 && !event.shiftKey)//DOWN
      ) {
        event.preventDefault();
        cellSize--;
        redrawAtEnd=true;
      ...
      if (
        (interactiveKernelBW && event.keyCode == 38 && event.shiftKey)//UP
      ) {
        event.preventDefault();
        kernelBW++;
        redrawAtEnd=true;
      ...
      if (
        (interactiveKernelBW && event.keyCode == 40 && event.shiftKey)//DOWN
      ) {
        event.preventDefault();
        kernelBW--;
        redrawAtEnd=true;
  ...
  panel.getCoordBBInView= ()=>{
    ...
    return [topLeftCoord[0],topLeftCoord[1],bottomRightCoord[0],bottomRightCoord[1]]
  };
  ...
  panel.getDataInView = ()=>{
    ...
    return data.filter(row=>{
      const location=getLocationFn(row);
        return location[0] >= topLeftCoord[0] &&
  ...
  if (initFn)
    initFn(grid.flat(), cellSize, global, panel);
  ...
  panel.addDrawFn(function(div,e){
    ...
    if (alwaysReaggregate || !previousTransform || previousTransform.x!=transform.x || previousTransform.y!=transform.y ||previousTransform.z!=transform.z || previousCellSize!==cellSize||previousKernelBW!==kernelBW||previousMaupReduce!==reduceMaup){
      ...
      grid = _generate2DHeatMap({
        ...
        offSetCoordRef,
        discretisationMode,
        reduceMaup,panel,offSetMaupMode
      });
  ...
  if (global.mouseX && global.mouseY){
      const [col,row]=discretiser.getColRow(global.mouseX-grid.xOffset,global.mouseY-grid.yOffset);
      if (
        col > 0 &&
        row > 0 &&
        col < grid.length &&
    ...
    if (scaleParams){
      if (showScaleParams){
        ...
        const panelW=40+d3.max(labels.map(s=>panel.ctx.measureText(s).width))+5;
        ...
        for (const scaleParam of scaleParams){
          ...
          if (scaleParam.getAutoscale())
            ...
            panel.ctx.strokeRect(30+5,c*10+5+1,9,8);  
